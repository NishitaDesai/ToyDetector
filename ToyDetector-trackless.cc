...
bool ToyDetector::getObjects(const Event& event){
	...
	// Count the tracks inside jet
	vector<int> part = slowJet.constituents(j);
	bool hasTrack = false;
	double ptsum = 0.0;
	for(int ipart = 0; ipart < part.size(); ipart++)
	  if(event[part[ipart]].isCharged()) ptsum += event[part[ipart]].pT();
	...
	if(!hastrack) tracklessjets.push_back(pnew);
